...
**⚠️ CLI-mode pitfall**: cron jobs only run while the Hermes process is
alive. If the user exits the CLI (`/quit` or closes the terminal), cron
stops ticking. For always-on cron, the user needs either the gateway
running as a systemd service (`hermes gateway install && hermes gateway
start`) or a messaging platform like Telegram connected. If the user says
"退出hermes就没cron了对吗" — the answer is yes, CLI cron dies with the
process. Offer alternatives: gateway background service, Telegram
delivery, or offline reference files the user checks manually.
