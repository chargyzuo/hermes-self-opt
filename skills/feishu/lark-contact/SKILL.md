...
批量查同事的个人状态 / 个性签名(先用 schema 看参数)。详见 [`references/lark-contact-user-profiles.md`](references/lark-contact-user-profiles.md)。
...
> **scope**: `profile:user_profile:read`（不在 contact 权限组内）；仅 `--as user` 支持。修改签名/状态目前无公开 API——`PATCH /open-apis/contact/v3/users/:user_id` 不能改这两个字段。
...
- **个人签名/状态 ≠ contact-v3 user PATCH**：`user_profiles`（profile-v2 项目）和 `PATCH /contact/v3/users/:user_id` 是两个独立 API 组，前者管 description/personal_status，后者管姓名/邮箱/部门等。不要混淆。
