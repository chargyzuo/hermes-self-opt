...
> ⚠️ **Pitfall**: `--detail with-ids` 和 `--detail full` **仅在 `--doc-format xml`（默认）下生效**。与 `--doc-format markdown` 组合时，CLI 会忽略 detail 选项并退回到 markdown 输出，同时在 warnings 中提示 `"--detail with-ids is only supported with --doc-format xml; returning markdown output and ignoring the unsupported detail option"`。需要 block ID 时，必须用 XML 格式获取，然后切回 markdown 做可读性输出。
